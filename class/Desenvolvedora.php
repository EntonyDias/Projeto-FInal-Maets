...
class Desenvolvedora{
   ...
   public function registrar($nome, $cnpj, $email, $senha){
      $query = "INSERT INTO " . $this->table_name . " (nomeDes, cnpjDes, emailDes, senhaDes) VALUES (?, ?, ?, ?)";
   ...
   public function login($email, $senha){
      $query = "SELECT * FROM " . $this->table_name . " WHERE emailDes = ?";
      $stmt = $this->conn->prepare($query);
      $stmt->execute([$email]);
      $usuario = $stmt->fetch(PDO::FETCH_ASSOC);
      if ($usuario && password_verify($senha, $usuario['senhaDes'])) {
         return $usuario;
      }
      return false;
   }
   ...
   public function atualizar($id, $nome, $cnpj, $email, $senha){
      $query = "UPDATE " . $this->table_name . " SET nomeDes = ?, cnpjDes = ?, emailDes = ?, senhaDes = ? WHERE idDes = ?";
      $stmt = $this->conn->prepare($query);
